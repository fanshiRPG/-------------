...
import com.webdev.newtech.mapper.IKu2Mapper;
...
@RestController
@RequestMapping("productKu2")
public class Ku2Controller {
    @Autowired
    IKu2Mapper productMapper;


    @GetMapping("add")
    public AjaxResult add(Ku2 product) {
        product.setId(null);
    ...
    @GetMapping("edit")
    public AjaxResult editProduct(Ku2 product) {
    ...
    public AjaxResult get(@RequestParam(required = true) Integer id) {
        Ku2 product = productMapper.selectById(id);
    ...
    public AjaxResult list(@RequestParam(required = true) Integer pageNum,
        ...
        // 检查查询条件
        QueryWrapper<Ku2> wrapper = new QueryWrapper<>(); // 确保使用正确的实体类
        ...
        List<Ku2> list = productMapper.selectList(wrapper);
        PageInfo<Ku2> info = new PageInfo<>(list);
